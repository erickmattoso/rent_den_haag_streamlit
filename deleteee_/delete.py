...
def page_settings():
    ...
    ______ = list(final['city'].unique())
    Plaats = st.multiselect('city', options=______, default=______)
    final = final[final['city'].isin(Plaats)]
    row1, row2 = st.columns(2)
    row1.write(final_, unsafe_allow_html=True)
    row2.write(map)
    st.dataframe(final.style.highlight_max(axis=0))
